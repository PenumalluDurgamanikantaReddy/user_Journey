...
function HomeContent() {
  ...
  // Extract just the YYYY-MM part safely to prevent '2020-01-01-01' bugs
  const startMonthStr = filters.dateRange.start ? filters.dateRange.start.slice(0, 7) : null;
  const startDate = startMonthStr ? `${startMonthStr}-01` : undefined;
  
  let endDate = undefined;
  const endMonthStr = filters.dateRange.end ? filters.dateRange.end.slice(0, 7) : null;
  if (endMonthStr) {
    const end = new Date(`${endMonthStr}-01`);
    end.setMonth(end.getMonth() + 1);
    end.setDate(0); // gets the last day of the month
